...
var f=-e.x+i.x;i=-e.y+i.y;e=Math.max(Math.sqrt(f*f+i*i),0.01);c=c(e,d)/e;f*=c;i*=c;a.change_vel(f,i);b.change_vel(-f,-i)}function Ca(a,b){return-Math.sqrt(a)/b}function Da(a,b){var c=b*b;return c*c/(a*a)}function V(a){var b;a=a.split("\n").filter(function(c){return c.length>0}).map(function(c){return c.split(",")});b=a[0];a=a.slice(1).map(function(c){return[b[c[0]],b[c[1]],c[2]]});ja({vertices:b||[],edges:a||[]})}function ka(){var a=[];a.push(g.map(function(b){return b.label}));a=a.concat(h.filter(function(b){return b.label}).map(function(b){return[g.indexOf(b.node1),
